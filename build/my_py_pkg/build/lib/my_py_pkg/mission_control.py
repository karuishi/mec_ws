...
class MissionControlNode(Node): 
    ...
    def __init__(self):
        ...
        self._publisher = self.create_publisher(MissionStatus, 'control_panel', 10)
        # create the clients
    ...
    def mission_control_loop(self):
        match (self._status):
            ...
            case MissionControlNode.WAITING_FOR_TRAINING:
                self.publish_control_panel('Waiting for Training...')    
            case MissionControlNode.TRAINED:
                self.publish_control_panel('Trained for '+self._nameOfPOI+'. Bill will start the spin')  
                self.call_executeSpin_service() 
                self._status=MissionControlNode.WAITING_FOR_SPIN            
            case MissionControlNode.WAITING_FOR_SPIN:
                self.publish_control_panel('Waiting for spin to finish...')   
            case MissionControlNode.TURNED:
                self.publish_control_panel('Turned. Lets start looking for '+self._nameOfPOI)
            ...
            case MissionControlNode.WAITING_FOR_RECOGNITION:
                self.publish_control_panel('Waiting for Recognition...')   
            case MissionControlNode.FOUND:
                self.publish_control_panel('Found '+self._nameOfPOI+' _nameOfPOI')
            ...
            case MissionControlNode.WAITING_FOR_FINAL_REMARKS:
                self.publish_control_panel('Waiting for Final Remarks...')                  
            case MissionControlNode.MISSION_ACCOMPLISHED:   
                self.publish_control_panel('Mission Accomplisded. Bye.')     
                exit()

    def publish_control_panel(self, message):
        msg=MissionStatus()
        msg.status=message
